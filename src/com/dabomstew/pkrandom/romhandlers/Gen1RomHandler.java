...
public class Gen1RomHandler extends AbstractGBRomHandler {
			...
			if (!usedOffsets.contains(offset)) {
				...
				for (int a = 0; a < 2; a++) {
					...
					if (rate > 0) {
						...
						thisSet.recommendedLevel = (int) (1.2 * average_level + 0.5);
	...
	public void randomizeBlaineQuiz(List<Integer> moveIndices) {
		...
		if(RandomSource.random() < 0.5) {
			// Correct move
			move = moveNames[moveNumToRomTable[moveIndices.get(tm - 1)]];
			answers[4] = 1;
		} else {
			// Wrong move
			int fakeTM = tm;
			...
			move = moveNames[moveNumToRomTable[moveIndices.get(fakeTM - 1)]];
			answers[4] = 0;
		}
